...
UNDO="UNDO_repasses_resumo_${TS}.sh"
...
# 1) Injeta a área de resumo abaixo dos filtros (logo antes da tabela)
...
# 2) JS que recalcula sempre que a tabela muda ou filtros mudam
js = r'''
<script>
(function(){
  ...
  function recompute(){
    ...
    tabela.querySelectorAll('tr').forEach(tr=>{
      ...
      if (tds.length >= 5 && tr.offsetParent !== null) {
        // mesma convenção do export PDF: 0 data, 1 colaborador, 2 desc, 3 valor, 4 status
  ...
  // se existir um botão "Aplicar filtros", observe também
  const btnAplicar = document.getElementById('btn-aplicar-filtros');
  if(btnAplicar) btnAplicar.addEventListener('click', ()=>setTimeout(recompute, 80));

  // fallback: recalcular após 1s (ajuda em loaders assíncronos)
...
print("OK: resumo por colaborador adicionado")
PY
