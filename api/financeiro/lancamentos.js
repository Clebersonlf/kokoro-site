...
export default async function handler(req, res) {
  ...
  // 1) Garante tabela SEM depender de extensões (id gerado no Node)
  await sql`
  ...
  try {
    ...
    if (req.method === 'POST') {
      // Tenta ler body como objeto; se vier string, faz parse.
      ...
      const { rows } = await sql`
        ...
        VALUES (${id}, ${aluno_id}, ${tipo}, ${Number(valor)}, ${descricao}, ${data})
