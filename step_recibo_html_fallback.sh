...
# Chama a API e guarda JSON
JSON_FILE="_recibo_tmp.json"
...
ARQ_BASE="recibo_cesar_${VALOR//./-}_$(date +%Y-%m-%d_%H-%M)_ok2"
...
# Se .html vier vazio/nulo, gera um HTML fallback simples a partir do texto
if [ -z "$HTML" ] || [ "$HTML" = "null" ]; then
  python3 - "$TXT" "$HTML_OUT" <<'PY'
import sys, html
texto = sys.argv[1]
dest  = sys.argv[2]
# quebra linhas do texto em <br> preservando acentos
body = "<br>".join(html.escape(texto).splitlines())
tpl = f"""<!doctype html>
<html lang="pt-BR">
<head>
  <meta charset="utf-8">
  <title>Recibo de Repasse</title>
