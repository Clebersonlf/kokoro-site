...
echo ">> Reescrevendo $F com correções (template string e colunas)"
cat > "$F" <<'JS'
...
export default async function handler(req, res) {
  ...
  try {
    // 1) Confere matrícula ativa e obtém professor (se existir na tabela)
    ...
    const professorId = mRows[0].professor_id || null;
    ...
    // 4) Insere pagamento
    // OBS: sua tabela 'pagamentos' não tem 'professor_id' (pelo histórico),
    // então não incluímos essa coluna aqui.
    ...
    // 5) Registra devido ao professor em rateios_pagamento
    // Colunas existentes (segundo seu banco): 
    // id, pagamento_id, professor_id, parte_professor, parte_titular,
    // regra_aplicada, criado_em, percentual_professor, percentual_titular, valor_professor
    // Não existem: valor_total, valor_titular. 
    // A coluna 'regra_usada' foi criada por você como gerada (não precisa inserir).
...
echo "OK: arquivo corrigido e sintaxe válida."
echo "Próximo passo: fazer deploy para produção."
